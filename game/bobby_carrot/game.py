    ...
    def update_texture_position(self, frame: int, map_data: List[int]) -> Tuple[Rect, Rect]:
        ...
        if step == 6 and is_walking and self.next_state == State.Death:
            ...
            if self.next_state is not None:
                # Consume the queued movement before updating so any next_state
                # set by update_dest (e.g., death on crumble tile) is preserved.
                queued_state = self.next_state
                self.next_state = None
                self.update_state(queued_state, frame, map_data)
...
def main():
    ...
    while running:
        ...
        # right edge of the visible area in screen-space pixels (used for HUD)
        hud_right = WIDTH if full_view else VIEW_WIDTH
        ...
        if map_info.carrot_total > 0:
            ...
            icon_width = 34
        screen.blit(assetsurfs.hud, (hud_right - (icon_width + 4), 4), icon_rect)
        num_10 = num_left // 10
        num_01 = num_left % 10
        screen.blit(assetsurfs.numbers,
                    (hud_right - (icon_width + 4) - 2 - 12,
                     4 + 14),
                    Rect(num_01 * 12, 0, 12, 18))
        screen.blit(assetsurfs.numbers,
                    (hud_right - (icon_width + 4) - 2 - 12 * 2 - 1,
                     4 + 14),
